...
`,L=[{path:"/",render:e=>{e.innerHTML=y()}},{path:"/login",guard:()=>!i.isLoggedIn(),redirect:"/",render:(e,t)=>{e.innerHTML=p(),g(e,t)}},{path:"/profile",guard:()=>i.isLoggedIn(),redirect:"/login",render:e=>{e.innerHTML=v("/profile"),x(e)}},{path:"404",render:e=>{e.innerHTML=w()}}];function S(e){L.forEach(({path:t,guard:s,redirect:o,render:r})=>{e.addRoute(t,n=>{if(s&&!s()){e.navigateTo(o);return}r(n,e)})})}function $(e,{isHashMode:t=!1}={}){const s={};let o="";function r(){return t?window.location.hash.replace(/^#/,"")||"/":window.location.pathname||"/"}function n(a){const d=s[a];d?d(e):s[404]&&s[404](e)}function l(){o=r(),n(o)}function f(a){t?window.location.hash=a:(history.pushState({},"",a),l())}function m(a,d){s[a]=d}function b(){o=r(),n(o),t&&window.addEventListener("hashchange",l),window.addEventListener("popstate",l)}return{addRoute:m,navigateTo:f,start:b,getCurrentPath:r}}function E(e=!1){const t=document.getElementById("root");if(!t)return;console.log("production");const s=$(t,{isHashMode:e});i.init(),i.subscribe(()=>{s.start()}),S(s),h(s),s.start()}E();
