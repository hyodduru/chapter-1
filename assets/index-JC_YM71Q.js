...
`,l={user:null,listeners:new Set,init(){const e=localStorage.getItem("user");e&&(this.user=JSON.parse(e))},setUser(e){this.user=e,localStorage.setItem("user",JSON.stringify(e)),this.notify()},logout(){this.user=null,localStorage.removeItem("user"),this.notify()},getUser(){return this.user},isLoggedIn(){return!!this.user},subscribe(e){return this.listeners.add(e),()=>this.listeners.delete(e)},notify(){for(const e of this.listeners)e(this.user)}},h=!!window.location.pathname.includes("index.hash.html"),i=h?"":"/front_5th_chapter1-1";function v(e,t){const s=e.querySelector("#login-form");s&&s.addEventListener("submit",o=>{o.preventDefault();const r=s.querySelector("input[type='text']").value;if(!r)return;const n={username:r,email:"",bio:""};l.setUser(n),t.navigateTo(`${i}/`)})}function x(e){document.addEventListener("click",t=>{if(t.target.tagName==="A"){const s=t.target.getAttribute("href");s&&s.startsWith("/")&&(t.preventDefault(),t.target.id==="logout"&&l.logout(),e.navigateTo(s))}})}function u(e="/"){const t=l.isLoggedIn(),s=o=>e===o?"text-blue-600 font-bold":"text-gray-600";return`
...
`;function w(e){const t=e.querySelector("#profile-form");t&&t.addEventListener("submit",s=>{s.preventDefault();const o=t.querySelector("#username").value,r=t.querySelector("#email").value,n=t.querySelector("#bio").value;if(!o)return;const a={username:o,email:r,bio:n};l.setUser(a),alert("프로필이 업데이트되었습니다.")})}const $=()=>`
  <div class="bg-gray-100 min-h-screen flex justify-center">
    <div class="max-w-md w-full">
      ${u()}
      <main class="p-4">
...
`,S=[{path:`${i}/`,render:e=>{e.innerHTML=$()}},{path:`${i}/login`,guard:()=>!l.isLoggedIn(),redirect:`${i}/`,render:(e,t)=>{e.innerHTML=g(),v(e,t)}},{path:`${i}/profile`,guard:()=>l.isLoggedIn(),redirect:`${i}/login`,render:e=>{e.innerHTML=y("/profile"),w(e)}},{path:"404",render:e=>{e.innerHTML=L()}}];function E(e){S.forEach(({path:t,guard:s,redirect:o,render:r})=>{e.addRoute(t,n=>{if(s&&!s()){e.navigateTo(o);return}r(n,e)})})}function P(e,{isHashMode:t=!1}={}){const s={};let o="";function r(){return t?window.location.hash.replace(/^#/,"")||"/":window.location.pathname||"/"}function n(d){const c=s[d];c?c(e):s[404]&&s[404](e)}function a(){o=r(),n(o)}function m(d){t?window.location.hash=d:(history.pushState({},"",d),a())}function b(d,c){s[d]=c}function p(){o=r(),n(o),t&&window.addEventListener("hashchange",a),window.addEventListener("popstate",a)}return{addRoute:b,navigateTo:m,start:p,getCurrentPath:r}}function I(e=!1){const t=document.getElementById("root");if(!t)return;const s=P(t,{isHashMode:e});l.init(),l.subscribe(()=>{s.start()}),E(s),x(s),s.start()}export{I as i};
